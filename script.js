...
const wifeName = "Flku";      // yahan uska real naam
...
if (prevBtn) {
  prevBtn.addEventListener("click", () => {
    if (currentPage > 0) {
      ...
      if (card) card.scrollTop = 0; // naya page pe top se
    }
  });
}
...
// ====== Heart burst (reuse for game + scroll end) ======
...
// ====== NEW: scroll end pe hearts confetti ======
let scrollHeartsShown = false;
...
if (card) {
  card.addEventListener("scroll", () => {
    ...
    // sirf LAST PAGE pe & bottom pe
    if (nearBottom && !scrollHeartsShown && currentPage === totalPages - 1) {
      scrollHeartsShown = true;
      showHeartBurst();

      // thodi der baad phir se allow
      setTimeout(() => {
        scrollHeartsShown = false;
      }, 2500);
    }
  });
}
